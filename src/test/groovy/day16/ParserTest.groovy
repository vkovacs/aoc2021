...
class ParserTest {
    ...
    void parseOnlyLiteralMessage() {
        assert new Packet(header: new Packet.Header(version: 6, typeID: 4), length: 21, value: 2021, subPackets: []) == parser.parseMessage("110100101111111000101000").get(0)
    }
    ...
    void parseOperatorLenType0() {
        ...
        assert packet == parser.parseMessage("00111000000000000110111101000101001010010001001000000000").get(0)
    ...
    void parseOperatorLenType1() {
        ...
        assert packet == parser.parseMessage("11101110000000001101010000001100100000100011000001100000").get(0)
    }

    @Test
    void nestedOperators() {
         assert 16 == parser.parseMessage(parser.hexToBin("8A004A801A8002F478")).get(0).sum()
    }

    @Test
    void multipleSubPackets() {
        assert 12 == parser.parseMessage(parser.hexToBin("620080001611562C8802118E34")).get(0).sum()
    }

    @Test
    void multipleSubPacketsDifferentLengthTypeId() {
        assert 23 == parser.parseMessage(parser.hexToBin("C0015000016115A2E0802F182340")).get(0).sum()
    }

    @Test
    void nestedOperatorsMultipleLiteral() {
        assert 31 == parser.parseMessage(parser.hexToBin("A0016C880162017C3686B18A3D4780")).get(0).sum()
    }
    ...
    void evalGreaterThan() {
        assert 1L == parser.parseMessage(parser.hexToBin("D8005AC2A8F0")).get(0).eval()
    }
